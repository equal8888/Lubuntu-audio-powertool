...
currentaltsamplerate=$(grep "# defaults.pcm.dmix.format" /usr/share/alsa/alsa.conf) && sudo sed -i "/${currentaltsamplerate}/ c defaults.pcm.dmix.format 'unchanged'" /usr/share/alsa/alsa.conf &&
